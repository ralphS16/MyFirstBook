...
\begin{defn}[Monoid action]
    ...
    Conversely, a map $\sigma: M \rightarrow \Perm_S$ that satisfies $\sigma(1_M) = \id_S$ and $\sigma(xy)= \sigma(x) \circ \sigma(y)$ for any $x,y \in M$ gives rise to a "monoid action" $\actmon_\sigma$ defined by $x \actmon_\sigma s = \sigma(x)(s)$.\footnote{These are inverse operations, i.e.:\[\sigma_{\actmon_{\sigma}} = \sigma \quad \text{ and } \quad \actmon_{\sigma_{\actmon}} = \actmon.\]}
\end{defn}
\begin{exmp}
...
\begin{defn}[Group action]
    Let $G$ be a "group" and $S$ a set, \AP an (left) ""action@@GRP"" of $G$ on $S$ is a (left) "monoid action" of $G$ on $S$.
\end{defn}
\begin{exmp}
...
\begin{defn}
    ...
    \begin{enumerate}
        ...
        \itemAP ""idempotent"" ($\forall x \in A, c(x) = c(c(x))$).\footnote{We will use this definition of "idempotence" in other contexts.}
    \end{enumerate} 
\end{defn}
...
\begin{defn}\label{defn:topspace}
	\AP A ""topological space"" is a pair $(X, \topo)$, where $X$ is a set and $\topo\subseteq \mP(X)$ is closed under arbitrary unions and finite intersections\footnote{For any family of "open sets" $\{U_i\}_{i \in I}\subseteq \topo$,\[\bigcup_{i \in I} U_i \in \topo,\] and if $I$ is finite,\[\bigcup_{i \in I} U_i \in \topo.\]} whose elements are called ""open sets"" of $X$. We call $\topo$ a \textbf{"topology"} on $X$.
