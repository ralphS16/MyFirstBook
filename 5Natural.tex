...
\begin{defn}[Natural transformation]\label{defnattran}
	\AP Let $F,G : \mathbf{C} \rightsquigarrow \mathbf{D}$ be two ("covariant") "functors", a "natural transformation" $\phi: F \Rightarrow G$ is a map $\phi: \obj{\mathbf{C}} \rightarrow \mor{\mathbf{D}}$ that satisfies $\phi(A) \in \Hom_{\mathbf{D}}(F(A), G(A))$ for all $A \in \obj{\mathbf{C}}$ and makes diagram \eqref{diag:nattrans} "commute" for any $f \in \Hom_{\mathbf{C}}(A,B)$:
...
\begin{defn}[The left action of functors]\label{defn:leftaction}
	...
	\begin{equation}\label{diag:commleftaction}
		...
		\end{tikzcd}
	\end{equation}
	Consider this diagram after removing all applications of $G$, by "naturality" of $\phi$, it is "commutative". Since "functors" "preserve" "commutativity", the diagram still "commutes" after applying $G$, hence $G\phi: G\circ F \Rightarrow G \circ F'$ is indeed "natural".
...
\begin{lem}
	Diagram \eqref{diag:etdc} "commutes", i.e.: $\eta H' \vertcomp G\phi = G'\phi \vertcomp \eta H$.
\end{lem}
\begin{proof}
...
\end{tikzcd}
\end{equation}
This follows from the "naturality" of $\eta$.\footnote{In \eqref{diag:nattrans}, replace $A$ with $H(A)$, $B$ with $H'(A)$, $f$ with $\phi(A)$, $F$ with $G$ and $G$ with $G'$.}
\end{proof}
...
\end{prop}
\begin{proof}
	Similarly to how we constructed diagram \eqref{diag:etdc} in $\catFunc{\mathbf{C}}{\mathbf{E}}$ previously, we can use the "biaction" of "functors" and "composition" of "functors" to obtain the following diagram in $\catFunc{\mathbf{C}}{\mathbf{E}}$.\footnote{All $\circ$'s are left out for simplicity.}
	\begin{equation}
	\begin{tikzcd}
	...
	\end{tikzcd}
	\end{equation}
	This "commutes" because each face of the cube corresponds to a variant of diagram \eqref{diag:etdc} (with some substitutions and application of a "functor") and combining "commutative" diagrams yields "commutative" diagrams.\footnote{Here is the rundown: Call Nat(phi,f,A,B) then use it.\begin{itemize}
        \item[\textbf{Top face:}]
        \item[\textbf{Bottom face:}]
        \item[\textbf{Left face:}]
        \item[\textbf{Right face:}]
        \item[\textbf{Front face:}]
        \item[\textbf{Back face:}]
    \end{itemize}} Then, it follows easily that $\horcomp$ is associative.%TODO: do it!
\end{proof}%TODO: instructions on how to follow proofs with NAT(,,,) earlier.
...
\begin{prop}[Interchange identity]\label{prop:interchange}
	...
	\end{equation}
    \marginnote{It is in the drawing of \eqref{diag:interidsetting} that the intuition behind "vertical" and "horizontal" is taken.}
...
\begin{proof}
	...
	\end{align*}
	Combining the factored diagram with \eqref{diag:rhsinterid}, we obtain \eqref{diag:interid} from which the "interchange identity" readily follows.
...
\begin{defn}[Strict $2$--cateory]\label{defn:2cat}
	...
	\begin{itemize}
		...
		\item for every $A,B \in \obj{\mathbf{C}}$ a "category" $\mathbf{C}(A,B)$ with $\Hom_{\mathbf{C}}(A,B)$ as its "objects" ("composition" is denoted $\vertcomp$ and "identities" $\one$) and "morphisms" are called $2$--\textbf{morphism},
...
\begin{exer}\label{exer:natural:equivfunccat}
	Let $\mathbf{C} \eqCat \mathbf{C}'$ and $\mathbf{D} \eqCat \mathbf{D}'$ be "equivalent" "categories", then $\catFunc{\mathbf{C}}{\mathbf{D}} \eqCat \catFunc{\mathbf{C}'}{\mathbf{D}'}$.\marginnote{\hyperref[soln:natural:equivfunccat]{See solution.}}
\end{exer}
\end{document}