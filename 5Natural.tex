...
\begin{defn}[Strict $2$--cateory]\label{defn:2cat}
	...
	\end{itemize} 
	such that the "interchange identity" \eqref{eqn:interid} holds.\footnote{The "interchange identity" does not come out of nowhere, it is equivalent to the composition $\diamond$ being a "functor" $\mathbf{C}(B,C) \cattimes \mathbf{C}(A,B) \rightsquigarrow \mathbf{C}(A,C)$ for every $A,B,C \in \obj{\mathbf{C}}$. We leave you to show this in the special case of the "$2$--category@2cat" of "categories" in Exercise \ref{exer:natural:compositionisfunc}.}
\end{defn}
...
\begin{exmps}\label{exmp:isocat}
	\begin{enumerate}
		...
		\begin{gather*}
			...
			\Curry{\Uncurry{F}}(f)(Y) = \Uncurry{F}(f,Y) = F(f)_Y
		\end{gather*}
		\begin{gather*}
