...
\begin{exer}\label{exer:natural:horcompidentity}\marginnote{\hyperref[soln:natural:horcompidentity]{See solution.}}
	Let $F,G: \mathbf{C} \rightsquigarrow \mathbf{D}$ and $\phi: F \Rightarrow G$ be a "natural transformation". Show that $\phi \horcomp \one_{F} = \phi F$ and $\one_{G} \horcomp \phi = G\phi$. Infer that $\one_{\id_{\mathbf{C}}}$ is the "identity" at $\mathbf{C}$ for $\horcomp$.
\end{exer}
