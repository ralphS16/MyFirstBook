...
\end{defn}
\AP As usual, there is an ""identity transformation"" $\one_F:F \Rightarrow F$, it sends every "object" $A$ to the "identity" map $\id_{F(A)}$. but let us go back to the group case. Although very specific to "single object" "categories", it is simple enough to quickly digest.
\begin{exmp}
	%TODO: example about transformations between functors where the source category is discrete.
\end{exmp}
...
\end{exmp}
%TODO: example, tautological transformation from the source functor to the target functor.
...
\begin{exmps}\label{exmp:isocat}
	\begin{enumerate}
		Here are other examples of "isomorphic@@CAT" "categories" that we have already seen and a couple of new ones.
		\marginnote[2\baselineskip]{Another example for readers who know a bit of advanced algebra. Let $k$ be a "field" and $G$ a finite "group", the "categories" of $k[G]$--\href{https://en.wikipedia.org/wiki/Module_(mathematics)}{modules} ($k[G]$ is the \href{https://en.wikipedia.org/wiki/Group_ring}{group ring} of $k$ over $G$) and of $k$--\href{https://en.wikipedia.org/wiki/Group_representation}{linear representations} of $G$ are "isomorphic@@CAT".}
...
\begin{exmp}[Medium]
	...
	\end{equation}\end{marginfigure}
\end{exmp}
\begin{exer}\label{exer:natural:equivsetandequiv}\marginnote{\hyperref[soln:natural:equivsetandequiv]{See solution.}}
	%TODO: show that category of sets and functions is equivalent to the category of sets and equiv relation and functions preserving the equiv relation. 
\end{exer}
