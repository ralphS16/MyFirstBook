...
%TODO: Read this.
%TODO: this chapter is exceptional because we will not spend any time on dual definitions and result. This is because of my background. For a gentle introduction to comonads: go read Paolo's notes.%TODO: monads in spans are categories.%TODO: put this before adjunctions.
...
With an arbitrary functor $F$, you might have a hard time to come up with a way to "compose" two Kleisli morphisms $A \rightarrow FB$ and $B \rightarrow FC$ or even define the "identity" "Kleisli morphism" $A \rightarrow FA$, but the data of a "monad" lets you do just that. Indeed, given $f: A \kmor B$ and $g: B \kmor C$, while $g$ is not "composable" with $f$, $Mg$ is so we have $Mg \circ f : A \rightarrow MMC$ and it suffices to apply the multiplication $\mu_C$ to obtain $\mu_C \circ Mg \circ f : A \kmor C$. \AP We denote $g \klcomp{M} f := \mu_C \circ Mg \circ f$ and call it the ""Kleisli composition"". Also, for any $A \in \obj{\mathbf{C}}$,the "component" of the "unit@@MND" at $A$ yields "Kleisli morphism" $\eta_A: A \kmor A$. Let us check that $\klcomp{M}$ is associative and that $\eta_A$ behaves like the "identity" with respect to $\klcomp{M}$.
