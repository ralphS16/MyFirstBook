...
\begin{exmps}[Concrete categories]\label{exmp:concreteCat}
	This second list of examples contains so-called "concrete categories". Informally, are "categories" of sets with extra structure, where "morphisms" are functions that preserve that extra structure.\footnote{Formally, see Definition \ref{defn:concrete}.}
	\begin{enumerate}
...
\begin{exer}\label{exer:catfunc:forgetfulcat}\marginnote{\hyperref[soln:catfunc:forgetfulcat]{See solution.}}
	Show that both assignments $\mathbf{C} \mapsto \obj{\mathbf{C}}$ and $\mathbf{C} \mapsto \mor{\mathbf{C}}$ yield "functors" $\catCat \rightsquigarrow \catSet$. Their action on "morphism" of "categories" (i.e. "functors") is straightforward: the first sends $F$ to $F_0$ and the second sends $F$ to $F_1$. Show that the "functor" $\obj{(\placeholder)}$ is not "faithful", but $\mor{(\placeholder)}$ is.
\end{exer}
...
\end{defn}
\begin{exer}[\NOW]\label{exer:catfunc:product2and2}\marginnote{\hyperref[soln:catfunc:product2and2]{See solution.}}
